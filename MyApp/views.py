...
def roi_offset(image, rect, offset, median_value, contours_poly, append, stop_header_image):
	...
	# Decode Barcode
	cv2.imwrite("./barcodeDetector/MyApp/detection_images/roi_image_1.jpg",roi_image) #./
	test_image1 = "./barcodeDetector/MyApp/detection_images/roi_image_1.jpg" #./
...
def roi_offset_less(image, rect, offset,median_value, contours_poly, append, stop_header_image):
	...
	# Decode Barcode
	cv2.imwrite("./barcodeDetector/MyApp/detection_images/roi_image_2.jpg",roi_image)	#./
	test_image2 = "./barcodeDetector/MyApp/detection_images/roi_image_2.jpg" #./
...
def rotate_image(contours_poly,roi_image):
	...
	if(abs(angle) >= 1): #if the angle is larger than 1. rotate the image
		...
		cv2.imwrite("./barcodeDetector/MyApp/detection_images/rotated.jpg",rotated) #./
	else:
		rotated = roi_image
...
def find_contour(roi_image_barcode):
	...
	cv2.imwrite("./barcodeDetector/MyApp/detection_images/roi_barcode_threshold_2.jpg",roi_barcode_thres) #./

	kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 3))
	closed = cv2.morphologyEx(roi_barcode_thres, cv2.MORPH_CLOSE, kernel)
	cv2.imwrite("./barcodeDetector/MyApp/detection_images/barcode_filtered_2.jpg", closed) #./
	...
	cv2.imwrite("./barcodeDetector/MyApp/detection_images/barcode_contour_image_2.jpg", draw_image)	#./
...
def write_to_image(image,detected_bounding_box,detected_information):
	...
	cv2.imwrite("./barcodeDetector/MyApp/detection_images/final_segmented_barcode.jpg", image) #./
